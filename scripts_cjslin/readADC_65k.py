...
def main(GPort,SPI,SERIAL0,FIFO,NWORD):
    ...
    if (NWORD%16) != 0 :
        print("ERROR!!  Number of bytes to read is not integral of 4096")
        sys.exit()

    nRead=int(NWORD/65536)
    ...
    ##########################################################################
    adcDataBlock=[]
    for ix in range (0,nRead):
        tempData = SPI.readbytes(65536)
        if ix >3 and ix< 5:
            printADCdata(deSerialize(tempData))
        adcDataBlock += tempData
...
#    idata=sys.argv[2]
    ...
    #Number of words in FPGA FIFO*16
    NWORDFIFO=4096*16
    print("Number of words to read",NWORDFIFO)
